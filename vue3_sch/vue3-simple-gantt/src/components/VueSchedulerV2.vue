        ...
        .resizable({
          ...
          listeners: {
            move(event) {
              ...
              const selectedEvent = propData.value.events[dataIndex];
              ...
              const fdate = formatDate(selectedEvent.start)
              console.log(fdate)

              //const match = selectedEvent.start.toString().match(
              const match = fdate.match(
                /(\d{4})\/(\d{2})\/(\d{2}) (\d{2}:\d{2}):\d{2}/
              );
              if (match) {
                //const [, startDay, startMonth, startYear, startTime] = match;
                const [, startYear, startMonth, startDay, startTime] = match;
                ...
                //propData.value.events[dataIndex].end = `${endTime.getDate()}/${
                //  endTime.getMonth() + 1
                //}/${endTime.getFullYear()} ${endTime.getHours()}:${endTime.getMinutes()}`;
                propData.value.events[dataIndex].end = endTime
              }
            },
          },
          modifiers: [
